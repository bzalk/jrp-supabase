...
with the local environment record before provisioning or reset. The VPS
installer writes that exact token into `/opt/jrp-supabase/docker/.env`; protected
Sync API requests must use it as `Authorization: Bearer <SYNC_API_TOKEN>`.
